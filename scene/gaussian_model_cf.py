...
class CFGaussianModel:
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().cuda()#[54946, 3]
        if self.view_dependent:
            fused_color = RGB2SH(torch.tensor(
                np.asarray(pcd.colors)).float().cuda())#[54946, 3]
        else:
            fused_color = torch.tensor(np.asarray(pcd.colors)).float().cuda()
        features = torch.zeros(
            (fused_color.shape[0], 150, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :150, 0] = fused_color[:, :150]
        features[:, 150:, 1:] = 0.0
        ...
        self._xyz = fused_point_cloud.detach().requires_grad_(False)
...
class CF3DGS_Render:
    def __init__(self, sh_degree=0, white_background=False,
    ...
    def render(
        ...
        if len(out) == 4:
            rendered_image, radii, rendered_depth, rendered_alpha = out#[3, 721, 1440];[54946];[1, 721, 1440];[1, 721, 1440]
