...
class ERA5Dataset(Dataset):
    def __init__(self, cfg, timestamps, data_root_dir="/project/peilab/dataset/era5_np_float32_part"):
        ...
        self.timestamps = [i.strftime("%Y-%m-%d %H:%M:%S") for  i in timestamps]
    ...
    def _pre_filter_valid_indices(self):
        """预先检查哪些时间戳的数据是完整的且对应的PLY文件不存在"""
        valid_timestamps = []
        ...
        for timestamp in self.timestamps:
            ...
            ply_path = os.path.join(
                'output',
                timestamp[:4],  # 年份作为子目录
                f"{timestamp}.ply"  # 时间戳作为文件名
            )
            
            # 两个条件：数据可用且PLY文件不存在
            data_available = self._check_data_available(timestamp)
            ply_not_exists = not os.path.exists(ply_path)
            
            if data_available and ply_not_exists:
                valid_timestamps.append(timestamp)
            else:
                # 打印详细的跳过原因
                if not data_available:
                    print(f"Skipping {timestamp} - ERA5 data not complete")
                if not ply_not_exists:
                    print(f"Skipping {timestamp} - PLY file already exists at {ply_path}")
        
        print(f"Found {len(valid_timestamps)} valid timestamps to process")
        return valid_timestamps
        
    def _check_data_available(self, timestamp):
        """检查某个时间戳的所有数据文件是否存在"""
        # 检查气压变量文件
        for vname in self.cfg['vnames']['pressure']:
            for height in self.cfg['pressure_level']:
    ...
    def setup_dataset(self):
        if self.model_cfg.data_type == "co3d":
            ...
            cfg = {'vnames': {
                   ...
                   'single': ['v10','u10','v100', 'u100', 't2m','tcc', 'd2m','msl','tp6h','ssr6h']
                    },
            ...
            timestamps = self.model_cfg.timestamps 

            dataset = ERA5Dataset(cfg, timestamps)
            if len(timestamps) == 2:
                self.timestamps = dataset.timestamps 
            
            self.timestamps = dataset._pre_filter_valid_indices()
    
            print("Total images: ", len(self.timestamps))
