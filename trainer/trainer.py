...
class ERA5Dataset(Dataset):
    def __init__(self, cfg, timestamps, data_root_dir="/dataset/era5_np_float32_part"):
        ...
        if len(timestamps) == 2:
            ...
            self.timestamps = timestamps.strftime("%Y-%m-%d %H:%M:%S").tolist()
        
        self.steps = 1
        self.level_mapping = [cfg['total_levels'].index(val) for val in cfg['pressure_level'] if val in cfg['total_levels']]
        self.max, self.min = self.get_max_min()  # Read channel-wise mean and std
        
    def __len__(self):
        return len(self.timestamps) 

    def __getitem__(self, idx):
        init_timestamp = self.timestamps[idx]

        input_data = self._load_data(init_timestamp)
...
class GaussianTrainer(object):
    ...
    def setup_depth_predictor(self,):
        ...
        if self.depth_model_type == "zoe":
            ...
            from submodules.MiDaS.midas.dpt_depth import DPTDepthModel
            midas = DPTDepthModel(
                    path=None,
            ...
            import cv2
            from torchvision.transforms import Compose
            from submodules.MiDaS.midas.transforms import Resize, NormalizeImage, PrepareForNet
            from submodules.MiDaS.midas import transforms
            self.depth_transforms = Compose([
    ...
    def setup_dataset(self):
        if self.model_cfg.data_type == "co3d":
            ...
            start_time = self.model_cfg.start_time
            end_time = self.model_cfg.end_time
            timestamps = [start_time, end_time]

            dataset = ERA5Dataset(cfg, timestamps)
            if len(timestamps) == 2:
                timestamps = pd.date_range(start=timestamps[0], end=timestamps[1], freq="6h")
                self.timestamps = timestamps.strftime("%Y-%m-%d %H:%M:%S").tolist()
    ...
    def prepare_custom_data(self, idx, down_sample=True,
        ...
        input_data = self.data._load_data(timestamp)
        ...
        image_np = (original_image_np - min_vals) / (max_vals - min_vals)

        #image_np = (original_image_np - self.data.mean) / self.data.std
        ...
        if load_depth:
            if idx not in self.mono_depth:
                depth_tensor = self.predict_depth(original_image_np)   
                # depth_tensor = self.depth_model.infer_pil(image_pil, output_type='tensor')
                depth_tensor[depth_tensor < self.near] = self.near
                self.mono_depth[idx] = depth_tensor.cuda()
            else:
        ...
        points = pts[0].permute(1, 2, 0).cpu().numpy().reshape(-1, 3)#pts[0].shape[3, 721, 1440]  points.shape(1038240, 3)
        viewpoint_camera = Camera(idx, R, t, FoVx, FoVy, color_torch,
                                gt_alpha_mask=None, image_name=timestamp,
                                intrinsics=self.intrinsic,
                                uid=idx, is_co3d=True)
        ...
        # if down_sample:
        #     voxel_size = 0.01
        #     while len(pcd_data.points)> 300_000:  
        ...
        # intervals = 1 + np.abs(row_indices - equator_row) // 8

        # rows, cols = [], []
        # for row, interval in enumerate(intervals):
        #     cols_row = np.arange(0, 1440, interval)
        #     rows.append(np.full_like(cols_row, row))
        ...
        # cos_lat = np.cos(np.deg2rad(latitudes))  # shape: (721,)
        # intervals = 7 / cos_lat
        # intervals = np.clip(intervals, 7, 20).astype(int)  # shape: (721,)
        ...
        #     col_indices = np.arange(0, grid_data.shape[1], interval)
        #     sampled_data.append(grid_data[row, col_indices, :])
        ...
        for row in range(grid_data.shape[0]):
            ...
            sampled_row = grid_data[row, col_indices, :]
    ...
    def load_viewpoint_cam(self, idx, pose=None, load_depth=False):
        if self.data_type == "co3d":
            ...
            pdb.set_trace()
            original_image_np = np.load(image_name)
            height, width, _ = original_image_np.shape
            image_np = original_image_np / np.max(original_image_np)
            color_torch = torch.from_numpy(original_image_np / np.max(original_image_np)).permute(2, 0, 1).float()
