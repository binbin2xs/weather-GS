...
class CMIP6_ERA5PointCloudDataset(Dataset):
    def __init__(self, cfg, time_ranges):
        ...
        bucket_era5 =dict(bucket_name = 'your_bucket_name',
    ...
    def __getitem__(self, idx):
        ...
        era5 = self._load_data_era5(self.pc_timestamps[pc_idx])

        era5 = torch.from_numpy(era5).float()
        era5 = era5.unsqueeze(0).permute(0, 3, 1, 2)
        era5_inp = F.interpolate(era5, size=(32,64), mode='bilinear', align_corners=False)
        era5_gt = F.interpolate(era5, size=(64,128), mode='bilinear', align_corners=False)
        era5_inp = era5_inp.squeeze(0).permute(1, 2, 0).numpy()
        era5_gt = era5_gt.squeeze(0).permute(1, 2, 0).numpy()
    

        era5_inp = self._normalize_era5(era5_inp)
        era5_gt = self._normalize_era5(era5_gt)
       
        return era5_gt, era5_inp
