...
import pdb
from GSSAViT_fixscale_era5_era5 import SuperFormer_v2
from dataset_6hours_era5_era5 import CMIP6_ERA5PointCloudDataset
from scene.gaussian_model_cf_channel3 import CF3DGS_Render as GS_Render
from trainer.trainer_channel3 import GaussianTrainer
...
def get_dataset():
    ...
    time_ranges = [("1981-01-01 00:00:00", "2015-01-01 00:00:00")]
...
def main():
    ...
    criterion = L2_LOSS(reduction='mean',learn_log_variance=dict(flag=True, requires_grad=True, channels=3, logvar_init=0.),)
    ...
    for iteration, (gt, ori_weather) in enumerate(train_loader, start=start_iteration):
        if iteration >= args.total_iterations:
            break
        latitudes = np.linspace(-90, 90, 48)
        longitudes = np.linspace(-180, 180, 96)
        ...
        optimizer.zero_grad()
        outputs, gt = model(ori_weather.permute(0, 3, 1, 2), gt.permute(0, 3, 1, 2))
        
        gs_outputs = torch.cat([xyz, outputs], dim=2)
        _, pcd, viewpoint_cam = GaussianTrainer.prepare_custom_data(gs_outputs.squeeze(0), 0.089, orthogonal=True, down_sample=True)
        ...
        loss = criterion(outputs.unsqueeze(0), gt)
        loss.backward()
        ...
        if iteration % 10000 == 0 and iteration > 0 and rank == 0:
