...
class Attention(nn.Module):
    def __init__(
            self, dim, num_heads=8, qkv_bias=False, window_size=None, rel_pos_spatial=False, enable_scale_attn=False):
        super().__init__()
    ...
    def forward(self, x, H, W, scale_embed):
        B, N, C = x.shape
        if self.enable_scale_attn:
            resi = x
            scale_embed = scale_embed.unsqueeze(1).unsqueeze(2).repeat(1, 1, N, 1)  # b, h_count*w_count, num_gs_seed, channel
            scale_embed = scale_embed.reshape(B, -1, C) # b*h_count*w_count, num_gs_seed, channel
            x, _ = self.gs_cross_attn_scale(x, scale_embed, scale_embed)
            x = resi + x
            resi = x
            x = self.norm1(x)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, H, W, scale_embed):
        ...
        B = scale_embed.shape[0]
        resi = x
        scale_embed = scale_embed.unsqueeze(1).unsqueeze(2).repeat(1, int(B_w // B), int(N_w), 1)  # b, h_count*w_count, num_gs_seed, channel
        scale_embed = scale_embed.reshape(B_w, -1, C) # b*h_count*w_count, num_gs_seed, channel
        x, _ = self.gs_cross_attn_scale(x, scale_embed, scale_embed)
        x = resi + x
        resi = x
        x = self.norm1(x)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False,
                 ...
                 window_size=None, window=False, rel_pos_spatial=False, layer_idx=1):
        super().__init__()
        self.norm1 = norm_layer(dim)
        enable_scale_attn = (layer_idx <= 100)
        if not window:
            self.attn = Attention(
                dim, num_heads=num_heads, qkv_bias=qkv_bias,
                window_size=window_size, rel_pos_spatial=rel_pos_spatial, enable_scale_attn=enable_scale_attn,)
        else:
    ...
    def forward(self, x, H, W, scale_embed, mask=None):
        x = x + self.drop_path(self.attn(self.norm1(x), H, W, scale_embed))
...
class ViT(BaseModel):
    ...
    def __init__(self, 
        ...
        self.mlp_block_gaussian = nn.Sequential(
        ...
        self.mlp_block_channel = nn.Sequential(
            nn.Linear(embed_dim, embed_dim),
            nn.ReLU(),
            nn.Linear(embed_dim, embed_dim * 4),
            nn.ReLU(),
            nn.Linear(embed_dim * 4, 5)
        ) 
    ...
    def embedding_forward(self, x, scale, mask=None, **kwargs):
    ...
    def transformer_forward(self, 
        ...
        for i, blk in enumerate(self.blocks):
            if self.lms_checkpoint_train:
                if self.training:
                    a = (x, self.Hp, self.Wp, scale_embed)
                    x = checkpoint(blk, *a)
                else:
                    x = blk(x, self.Hp, self.Wp, scale_embed)
            else:
                x = blk(x, self.Hp, self.Wp, scale_embed)
    ...
    def decoder_forward(self, x):
        ...
        x_channel = self.mlp_block_channel(x)#.reshape(B, -1, 5) 
        x_gaussian = self.mlp_block_gaussian(x)#.reshape(B, -1, 8) 
        x = torch.cat([x_channel, x_gaussian], dim=-1)     
        return x


    def interpolate_data_2d(self, x, size=(128, 256)): 
        B, V, H, W = x.shape
        x = F.interpolate(x, size=size, mode='bilinear', align_corners=False)  # [B, V, H_t, W_t]
        return x

    def forward(self, x, gt, scale, **kwargs):
        # x = x[:, :self.in_chans, :, :]
        with torch.no_grad():
            x_interpolate = self.interpolate_data_2d(x, size=(gt.shape[2], gt.shape[3]))
      
        x, scale_embed = self.embedding_forward(x, scale, **kwargs)

        x = self.transformer_forward(x, scale_embed)
        
        x = self.decoder_forward(x)
       
        return x, gt, x_interpolate
...
class SuperFormer_v2(BaseModel):
    def __init__(self, 
        ...
        base_default_dict =dict(
            ...
            patch_size=patch_size, patch_stride=patch_stride, in_chans=in_chans, out_chans=out_chans, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4, qkv_bias=True,  norm_layer=partial(nn.LayerNorm,eps=1e-6), learnable_pos= True,
        )
    ...
    def forward(self, input, gt, scale, step=0, **kwargs):
        return  self.arch_net(input, gt, scale, **kwargs)
