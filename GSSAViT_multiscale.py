...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, rel_pos_spatial=False, enable_scale_attn=False):
        super().__init__()
        self.dim = dim
...
class ViT(BaseModel):
    ...
    def __init__(self, 
        ...
        self.out_linear = nn.Linear(embed_dim, 95, bias=False)
    ...
    def forward(self, x, gt, scale, **kwargs):
        x = x[:, :self.in_chans, :, :]
        with torch.no_grad():
            x = self.interpolate_data_2d(x, size=(128, 256))
            gt = self.interpolate_data_2d(gt, size=(int(721*scale), int(1440*scale)))
...
class SuperFormer_v2(BaseModel):
    def __init__(self, 
        ...
        base_default_dict =dict(
            ...
            patch_size=patch_size, patch_stride=patch_stride, in_chans=in_chans, out_chans=out_chans, embed_dim=768, depth=24, num_heads=12, mlp_ratio=4, qkv_bias=True,  norm_layer=partial(nn.LayerNorm,eps=1e-6), learnable_pos= True,
        )
